...
def test_sweep_against_fixture():
    ...
    assert row_010["refusal_recall"] == 0.0
    ...
    # At 0.25, 2 of 3 expected-refusal queries scored below threshold.
    assert row_025["refusal_recall"] == pytest.approx(2/3, abs=0.01)
    ...
    # At 0.40, all 3 expected-refusal queries scored below threshold.
    assert row_040["refusal_recall"] == 1.0


def test_recommend_picks_highest_combined_score():
    """The recommended threshold maximizes combined_score.

    Note: compute_sweep now computes combined_score as
    (precision + recall + retrieval_pass_rate) / 3. This test feeds a
    hand-crafted table without a recall column, so recommend_threshold
    falls back to (precision + pass_rate)/2 — still a valid test of the
    recommender's max-picking behavior on whatever combined_score it sees.
    """
    table = [
