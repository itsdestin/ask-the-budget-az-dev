...
| Create `app/routes/books.py` | "Add a JLBC book": catalog/discovery → bulk enqueue (Task 15) |
| Create `data/jlbc-book-sources/` + `scripts/build_book_catalog.py` + `data/jlbc-book-catalog.json` | Vendored mockup URL harvest → verified edition catalog (41 approps + 21 baseline editions) |
| Create `ingest/book_discovery.py` | `plan_edition` (catalog-first, HEAD-verified probe ladder) + `walk_edition` (dual index/TOC walk, host rewrites, classification) |
...
### Task 15: "Add a JLBC book" — catalog-seeded discovery + dual TOC walk

**REDESIGNED 2026-07-30 after auditing the website mockup's URL harvest** (`C:\Users\desti\JLBC Website Revamp\search-build\` — read its `BUILD.md` and `SEARCH-SYSTEM.md` first). Ground truth that killed the naive derive-and-walk design: 41 approps editions (FY1984–2026) + 21 baseline editions (FY2007–2027) across ~6 URL naming eras; the mockup's own build rule is **"never guess URLs — verify 200 or don't ship"**; per-agency filenames are non-derivable (roster shifts yearly); the agency-index PDF and the linked-TOC PDF yield **disjoint** children (per-agency vs statewide sections) so BOTH must be walked; book↔children share only a directory prefix, which breaks in the rolling `/budget/` years (FY2023 approps, FY2023/24 baseline links, FY2027 baseline); two dead legacy hosts appear inside old PDFs (`www.azleg.gov/jlbc/` — rewrite to azjlbc.gov, byte-identical paths — and `www.azleg.state.az.us/jlbc/`, which the mockup itself missed); casing is inconsistent *within* editions (IIS is case-insensitive; dedupe must be too); one AFR URL only works double-encoded (never re-encode); `/budget/apprpttoc.pdf` is un-year-stamped and will be silently repurposed.

**Files:** Create `data/jlbc-book-sources/` (vendored harvest), `scripts/build_book_catalog.py`, `data/jlbc-book-catalog.json` (generated, committed), `ingest/book_discovery.py`, `app/routes/books.py`; Modify `webapp/src/pages/Upload.tsx` (second panel), `api.ts`; Tests `tests/test_book_catalog.py`, `tests/test_book_discovery.py`, `tests/test_books_route.py`, Upload.test.tsx additions.

- [ ] **Step 1 — vendor the mockup's harvest** into `data/jlbc-book-sources/` (committed; same rationale as Plan 2's `webapp/reference/` vendoring): `search-build/live-books.json`, `agency-corpus.json`, `summary-corpus.json`, `.toc-urls.txt` (the 38 authoritative linked-TOC URLs), `.sum-status.txt` (liveness proof for all 650 summary URLs), `BUILD.md`, plus a generated `idx-manifest.txt` listing the `idx/` and `idx2/` filenames (the `idx2/` names encode each edition's TOC URL — every `__` is a `/`; do NOT vendor the PDFs themselves, ~9MB). Add a README marking read-only + snapshot date (2026-06-16).

- [ ] **Step 2 — failing tests for the catalog builder** (`tests/test_book_catalog.py`): running `scripts/build_book_catalog.py` produces `data/jlbc-book-catalog.json` with — exact-count pins — **41 approps + 21 baseline editions**; each edition keyed `{family}-fy{YYYY}` carrying `{single_file_url|null, linked_toc_url|null, agency_index_url|null, per_agency: [{code, url, title}], summary_sections: [{name, url, title}], ingestable: bool, era_note}`; ingestable ranges pinned (approps: children FY2005+, both formats FY2011+; baseline: children FY2012+); known gaps recorded (`FY2000/FY2001 approps absent; FY2027 approps expected-but-unharvested`); zero URLs on either dead host (both rewritten to `azjlbc.gov`); the 3 known `azleg.state.az.us` broken records rewritten; the 9 agency-index PDFs that leaked into `summary-corpus.json` as fake sections ("INDIVIDUAL AGENCY INDEX…") are used as index-URL evidence then EXCLUDED from `summary_sections`; biennial combined files (FY1996/97, FY1998/99) appear under both years pointing at one URL; URL comparison is case-insensitive everywhere.

- [ ] **Step 3 — implement the builder**: joins `live-books.json` (whole-book URLs, classified linked-vs-single-vs-supplement by the regexes ported from the mockup's `search.js::reportFormats` — linked ← `links|toc|table of contents|apprpttoc`, single ← `single file|approprpt`, supplements excluded ← `slide|comparison|spreadsheet|presentation|forecast`), `.toc-urls.txt` + decoded `idx2` names (TOC URL per edition), `agency-corpus.json` id prefixes `ag-{prefix}-{code}` (child rosters + directory prefixes), `summary-corpus.json` (sections). Run it; commit the generated catalog. Tests PASS.

- [ ] **Step 4 — failing tests for discovery** (`tests/test_book_discovery.py`): `plan_edition(family, fy, *, fetcher) -> EditionPlan`:
  - Catalog hit → returns the verified URLs + rosters, `source: "catalog"`, zero network calls.
  - Catalog miss (e.g. FY2028) → **candidate ladder**, each candidate HEAD-verified via the injected fetcher before acceptance, `source: "probed"`: baseline index ladder `/{yy}baseline/agencyindex.pdf → /{yy}baseline/{yy}BaselineAgencyIndex.pdf → /{yy}baseline/FY20{yy}BaselineAgencyIndex.pdf`; baseline links ladder `/{yy}baseline/{yy}baselinelinks.pdf → …BaselineLinks.pdf → /budget/{yy}baselinelinks.pdf`; approps ladders `/{yy}ar/… → /{yy}AR/… → /{yy}app/… → /budget/apprpttoc.pdf`; single-file ladders similarly.
  - **Rolling-`/budget/` guard**: a `/budget/*` candidate is accepted only if the walked PDF's child links actually resolve under the requested `{yy}` paths — a repurposed rolling file for a different year is rejected with a clear error, never silently ingested.
  - `walk_edition(plan, *, fetcher) -> list[DiscoveredDoc]` walks **BOTH** the agency index AND the linked TOC (children are disjoint by construction), using the existing PyMuPDF link-annotation walkers from `ingest/discovery.py`; classification by the mockup builders' filename regexes (agency ← `^[a-z]{3}(-[a-z]+)?$` or `^(jud|leg|uni)[a-z]+$`; summary ← `s{N}|bh{N}|bd{N}|{N}|{N}-{N}|capitaloutlay` + the documented one-offs; skip ← `toc|index|links|appr|baseline|book|summ|capital` in the agency pass); host normalization covers BOTH dead hosts and schemeless hrefs; URLs pass through verbatim (no re-encoding); case-insensitive dedupe. Fixture PDFs: vendor exactly two real index PDFs (one baseline, one approps) into `tests/fixtures/` for the walk tests.
  - Every discovered child is HEAD-verified before it appears in the plan; failures land in `EditionPlan.unreachable` (reported, not fatal).

- [ ] **Step 5 — routes** (`tests/test_books_route.py`): `GET /api/books/catalog` → the editions with their formats (Plan 5 wires the search page's Full-report chooser to this — replacing the 3-entry hand-verified `REPORT_FORMATS` map; noted, not done here); `POST /api/books/discover {family, fiscal_year}` → `{source, documents: […], count, unreachable: […]}` with no downloads; `POST /api/books/ingest {family, fiscal_year}` → 202, one queue job per document (doc_ids via `make_doc_id`; Task-10 dedup skips already-ingested: `{queued, skipped_existing, unreachable}`); discovery/probe failure → honest 502-style `detail`, nothing queued. Downloads happen inside jobs via `DownloadCache.fetch(url)`.

- [ ] **Step 6 — UI**: the "Add a JLBC book" panel gains an edition picker fed by `/api/books/catalog` — current-year additions AND historical backfill (any ingestable edition, approps FY2005+, baseline FY2012+) with honest cost copy: *"A full book takes overnight on office computers. Historical backfills are best run one book at a time."* Invariant 8 checkbox replaced by static notice (JLBC-published documents are public record by definition). Discover → "Found 112 documents for FY 2028 Baseline (2 unreachable — listed)" → Add all.

- [ ] **Step 7** — vitest + pytest PASS; commit `feat(ingest): Add-a-JLBC-book — catalog-seeded discovery, ladder-probed new editions, dual TOC walk`.

**Recorded for the handoff doc (Plan 5):** any large historical backfill (dozens of books) is smartest run on Destin's machine before departure — office CPUs make it a weeks-long background grind; the catalog + picker make it *possible* either way. Known upstream data quirks that ingest inherits gracefully: FY2024/25 approps summary-section titles were partially unextractable in the mockup harvest (the walk here uses PyMuPDF, which may recover them; if not, humanized filenames are the fallback); FY2027 approps will appear via the probe ladder once published.
