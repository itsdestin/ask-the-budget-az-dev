...
| S29 | **The upload flow asks what a human knows, not what the database stores — and "Other" is a first-class option.** The current page opens with a 13-item dropdown of internal slugs (`s-pdf`, `bh-pdf`, `bd-pdf`, `detailed-list-pdf`) that are JLBC file-naming conventions, not analyst language, with no indication of which file to fetch or from where. It is replaced by a short guided flow: **(1) What are you adding?** — a small set of plain-language cards (Baseline Book · Appropriations Report · Feed Bill · Executive Budget · Annual Financial Report · Agency Budget Request · Fiscal note · **Other document**), each stating in one line where the document is published, **which file to grab**, and the accepted format, with the S24 registry as the source of every word. The technical `doc_type` is DERIVED, never typed. JLBC book cards redirect to "Add a JLBC book" per S25. **(2) Pick the file** — the picker only accepts that type's formats (S24), so a docx-only type cannot be handed a PDF. **(3) Confirm what you're adding** — the app inspects the file before accepting it (page count, PDF structure tree, first-page text) and shows a **first-page thumbnail** plus the metadata it inferred **from content rather than filename**, because the Plan 3 live run proved filenames lie: a file named for AHCCCS was really the Industrial Commission's page. Mismatches are stated plainly before anything is queued ("You chose Feed Bill, but this is a 2-page PDF — Feed Bills are usually 300+ page Word documents. Continue anyway?"). Existing duplicate detection surfaces here as a normal branch, not an error. **(4) One sentence of what happens next**, with the honest processing time. **"Other document"** routes to the S26 general profile and says so in the analyst's words — searchable and citable, but extracted with a general-purpose profile rather than a tuned one, and labelled that way in results. It must never be the path of least resistance for a document that has a real type: it sits last, and the confirm step names the better-fitting type when one is detected. | The upload page is the ONLY part of this system a colleague touches while doing something irreversible to a shared corpus, and it is the surface most likely to be used by someone who has never been told how any of this works. Every guardrail here is cheaper than the failure it prevents — and the expensive failures are silent ones (S25's 400-page single-file book, a mis-stamped agency page) that succeed loudly and degrade retrieval quietly. Added 2026-07-31 at user request. |
| S30 | **Section-aware ranking (fiscal-note corpus), shipped inert and calibrated by sweep.** The first fiscal-note eval (2026-08-01) exposed a specific, repeatable failure: **a note's own Description out-ranks the Estimated Impact section that actually answers the question.** For the PSPRS query the target note's Description came back at rank 6 while the Impact section carrying the $123.6M figure never surfaced at all — the right *note* was found and the wrong *part* of it won. Mechanism: Description chunks are short summaries, so they are topic-dense and score well on both BM25 and dense similarity for almost any query about the bill. The fix rides the **existing S21 post-rerank boost seam** (`retrieval/recency.py::apply_recency_boost`, applied in `pipeline.py` after the full fused pool is reranked and before it is trimmed to `top_k`) — a section weight is structurally identical machinery, not a new stage. **Ship it at 0.0 like the recency boost was**, then calibrate by sweep against `eval/fiscal_note_queries.yaml` with a sibling of `eval/calibrate_recency.py`; the weight lands as measured data, never as a guess. **Two implementation constraints, both measured, not assumed:** (a) key on the **leaf** section, never the full path — the root varies across `Fiscal Note` / `FISCAL ANALYSIS` / `JLBC Fiscal Note` / **`Fisccal Notee`** (405 chunks, a typo in JLBC's own source documents), so path matching silently loses thousands of chunks; (b) **normalize the leaf** (case-fold + near-miss) — 71 chunks sit under `estimated imppact`, about 4% of the target section. Enabling data: `section_path` is already a schema field and is **99.7% populated** on this corpus (13,237/13,278), leaves distributing as analysis 2,538 · estimated impact 1,953 · description 1,860 · local government impact 1,366. **Deliberately open, for the calibration to answer:** whether the right instrument is *promoting* Estimated Impact or *demoting* Description (or a length normalization, if short-summary dominance is the real cause), because a blanket boost demotes every other section and "what does this bill do?" legitimately wants Description. The calibration set is thin — 11 non-refusal queries, 3 flagged for human review — so it can detect a regression but cannot finely tune a weight until the query set grows. | The failure is not a chunking bug and not a model weakness: the corpus, the metadata and the boost seam are all already in place, and the only thing missing until 2026-08-01 was a way to measure whether a change helped. Ranking the answer above the summary of the answer is the single highest-value retrieval improvement visible on this corpus. Added 2026-08-01 at user request. |
