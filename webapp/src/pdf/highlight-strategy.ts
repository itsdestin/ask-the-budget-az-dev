...
// is `TextLayerSearchStrategy`, which performs the text-layer search that
// used to live inline in PdfPage.tsx. The seam is here so the future #57
// follow-up (chunk→PDF coord map captured at ingest) can swap in a
// `CoordMapStrategy` without rewriting the component.
//
// The strict-bbox-only behavior is the spec's correctness change (shipped
// 2026-05-20, Wave E): when a chunk has a stored bbox, we NEVER search
// outside it. A miss surfaces as an empty result so the viewer can show the
// "couldn't pinpoint" badge — an honest miss instead of a silent wrong
// highlight. Before that change an unrestricted whole-page fallback drew a
// yellow rectangle over an unrelated dollar amount, which is worse than
// drawing nothing: it invites the analyst to trust the wrong number.
//
// Ported from web/lib/highlight-strategy.ts (Plan 4 Task 11). The only
// change is the import path for findNormalizedMatch, which Task 9 moved to
// webapp/src/chat/citation-extract.ts.
...
import { findNormalizedMatch } from "../chat/citation-extract";
