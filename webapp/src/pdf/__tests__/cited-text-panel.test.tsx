...
//
// Carried from web/tests/cited-text-panel.test.tsx (Plan 4 Task 11);
// assertions unchanged, only the import path differs.
...
describe("CitedTextPanel", () => {
  ...
  it("renders fallback when chunkText is empty", () => {
    const html = renderToString(
      <CitedTextPanel chunkText="" spanStart={0} spanEnd={0} sourceLabel="" />,
    );
  ...
  it("renders the whole chunk text without underline when spans are sentinel (0, claimLen)", () => {
    // Legacy sentinel range used by pre-resolved-offsets cites, and the
    // ordinary case for a search-page passage (nothing has claimed it yet).
    // The panel should still be useful — show the whole chunk, just no
    // underline.
    const html = renderToString(
      <CitedTextPanel
