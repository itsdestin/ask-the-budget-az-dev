...
interface CiteAckResult {
   ...
   *  the cite failed. The Python tool loop (harness/tools.py) never
   *  raises — it returns `{ok: false, error: ...}` JSON with an
   *  already-human-readable message, so it passes through unchanged. */
  failureReason?: string;
 ...
 *  raw text is a JSON envelope from the retrieval sidecar / Python
 *  tool loop. */
