// Single decision point for the mascot's pose/scene, ported verbatim (minus
// the "use client" directive and the `@/` import alias) from
// web/components/mascot/useMascotPose.ts.
...
/**
 * Reads chat state plus an explicit refusalActive flag. Refusal
 * auto-detection is still not built (it needs the faithfulness verifier),
 * so callers pass false.
 */
export function useMascotPose(
  state: ChatState,
